...
const getBlogs = async function (req, res) {
  try {
    let data = req.query;
    let filter = { isDeleted: false, isPublished: true };  //mandatory
    if (Object.keys(data).length > 0) {
      if (data.tags) {
    ...
    if (allBlogs.length == 0) {
      return res.status(404).send({ msg: 'NO BLOGS FOUND FOR SUCH ID' });
    }
...
const deletedByQuery = async function (req, res) {
  try {
    ...
    if (decodedToken.authorId != data.authorId)
      return res.status(403).send({ status: false, msg: 'YOU ARE NOT AUTHORIZED TO MAKE ANY CHANGES' });
